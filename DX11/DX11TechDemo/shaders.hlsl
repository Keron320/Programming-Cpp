...
VOut VShader(float4 position : POSITION, float4 color : COLOR, float2 texcoord : TEXCOORD, float3 normal : NORMAL)
{
	...
	output.color = color + ambient_light_colour + (directional_light_colour * diffuse_amount);
...
float4 PShader(float4 position : SV_POSITION, float4 color : COLOR, float2 texcoord : TEXCOORDS) : SV_TARGET
{
	return color * texture0.Sample(sampler0, texcoord); //Enable to have the red tone
	//return texture0.Sample(sampler0, texcoord, normal);
}
